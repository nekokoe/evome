...
//    private static DBConnector dbconn = GWTServiceImpl.getDBConn();
    ...
    public static Task addTask(Task task){
        ...
        String sql = "INSERT INTO `task` SET "
                ...
                + "task.create='" + Shared.sdformat(new Date()) + "',"
                + "task.modify='" + Shared.sdformat(new Date()) + "',"
                //+ "task.submit='" + Shared.sdformat(task.getSubmitDate()) + "',"                
    ...
    public static boolean editTask(Task task) {
        ...
        String sql = "UPDATE `task` SET "
                ...
                + "task.modify='" + Shared.sdformat(new Date()) + "',"
                + "task.submit='" + Shared.sdformat(task.getSubmitDate()) + "',"
                + "task.finish='" + Shared.sdformat(task.getFinishDate()) + "',"                
                + "task.parent='" + task.getParentUUID() + "',"
                + "task.config='" + task.getKaKsConfig().getUUID() + "',"
    ...
    public static int addResource(Resource res){
        ...
        String sql = "INSERT INTO `resource` SET "
                ...
                + "resource.create='" + Shared.sdformat(new Date()) + "',"
                + "resource.modify='" + Shared.sdformat(new Date()) + "',"
    ...
    public static Config addConfig(Config config){
        String sql = "INSERT INTO `kaksconfig` SET "
                ...
                + "kaksconfig.create='" + Shared.sdformat(new Date()) + "',"
                + "kaksconfig.modify='" + Shared.sdformat(new Date()) + "',"
    ...
    public static ArrayList<Task> userTasks(Account account, Task.Classify c){
        ...
        switch(c){
            ...
            case IN_PROGRESS:
                ...
                break;
            case TODAY:
                sql = sql + " AND TIME_TO_SEC(TIMEDIFF(task.create, NOW())) < 86400";
                break;
            case THIS_WEEK:
                sql = sql + " AND TIME_TO_SEC(TIMEDIFF(task.create, NOW())) < 604800";
                break;
            case THIS_MONTH:
                sql = sql + " AND TIME_TO_SEC(TIMEDIFF(task.create, NOW())) < 18144000";
                break;
            case THIS_YEAR:
                sql = sql + " AND TIME_TO_SEC(TIMEDIFF(task.create, NOW())) < 6622560000";
                break;
            case EARLIER:
                sql = sql + " AND TIME_TO_SEC(TIMEDIFF(task.create, NOW())) > 6622560000";
                break;
